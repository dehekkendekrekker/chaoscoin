...
const string strMessageMagic = "Chaoscoin Signed Message:\n";
...
{
    // Chaoscoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Chaoscoin: 840,000 blocks in ~1.6 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 24 * 60 * 60; // Chaoscoin: 1 day
static const int64 nTargetSpacing = 60; // Chaoscoin: 1 minute
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Chaoscoin: This fixes an issue where a 51% attack can change difficulty at will.
...
{
        static const int64 BlocksTargetSpacing = 60; // Chaoscoin: 1 minute
        static const unsigned int TimeDaySeconds = 24 * 60 * 60; // Chaoscoin: 1 day
...
{
    // Chaoscoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc3, 0xb9, 0xde }; // Chaoscoin: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ChaoscoinMiner : ProcessBlock, block not accepted");
    }
